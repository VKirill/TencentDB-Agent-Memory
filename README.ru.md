...
# 4. Добавь проекты для авто-extract в allowlist
...
| **L1** | Структурированные факты («Kirill использует TS strict», «деплой через PM2») с типом (`persona`/`episodic`/`instruction`) и приоритетом | PM2 tick если есть новые L0 turn'ы; или `claude-mem extract` вручную | ~$0.01-0.05 / tick (Hy3) или ~$0.10-0.30 (Sonnet 4.6) |
...
```bash
claude-mem init                       # инициализировать .claude/memory/ в cwd
claude-mem capture                    # читать {user, assistant} JSON со stdin → L0
...
claude-mem extract                    # запустить L1 → L2 → L3 один раз
claude-mem extract --dry-run          # перечислить сессии без LLM-вызовов
claude-mem extract --max-sessions 1   # обработать максимум N сессий за раз
claude-mem stats                      # статистика базы
claude-mem mcp serve                  # MCP-сервер на stdio (Claude Code вызывает это)
```
...
После каждой завершённой задачи оркестратор будет спавнить `claude-mem
...
**`claude-mem extract` пишет «OPENROUTER_API_KEY not set»**
