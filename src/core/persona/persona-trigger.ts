...
export class PersonaTrigger {
  ...
  async shouldGenerate(): Promise<TriggerResult> {
    ...
    if (
      ...
      const result: TriggerResult = { should: true, reason: "Cold start: first extraction done, scene files exist" };
    ...
    if (
      ...
      const result: TriggerResult = { should: true, reason: "Recovery: persona.md body missing or empty — regenerating" };
